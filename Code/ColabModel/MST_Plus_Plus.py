...
class MaskedLayerNorm2d(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, affine=True):
        ...
        if self.affine:
            # Initialize parameters for BHWC format where C is the last dimension
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: [B, H, W, C] input feature
            mask: Optional mask to indicate valid regions
        """
        # Create mask if not provided
        if mask is None:
            mask = (x.abs() > 1e-6).float()
        ...
        # Count valid elements along channel dimension, ensure at least 1 to avoid division by zero
        valid = mask.sum(dim=-1, keepdim=True).clamp(min=1.0)
        
        # Calculate mean only on valid elements
        mean = (x * mask).sum(dim=-1, keepdim=True) / valid
        
        # Calculate variance only on valid elements
        var = ((x - mean) ** 2 * mask).sum(dim=-1, keepdim=True) / valid
        
        # Normalize with improved numerical stability
        x_norm = (x - mean) / (torch.sqrt(var + self.eps) + 1e-12)
...
class PreNorm(nn.Module):
    def __init__(self, dim, fn):
        ...
        self.norm = MaskedLayerNorm2d(dim)
...
class MS_MSA(nn.Module):
    ...
    def forward(self, x_in):
        ...
        out = out_c + out_p
        out = out.masked_fill(zero_mask, 0)
        return out
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=21, out_channels=12, n_feat=12, stage=3):
        super(MST_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        modules_body = [MST(in_dim=12, out_dim=12,dim=31, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
    ...
    def forward(self, x):
        ...
        h = self.conv_out(h)
        h += x
        # h = h.masked_fill(zero_mask, 0)
        return h[:, :, :h_inp, :w_inp]
MODEL_CLASS = MST_Plus_Plus