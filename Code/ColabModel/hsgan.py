...
from hsgan_network_module import Conv2dLayer,ResidualDenseBlock_5C,PixelUnShuffle,PixelShuffle
...
class SpectralAttnBlock(nn.Module):
    ...
    def masked_global_avg_pool(self, x):
        """
        Custom masked global average pooling
        """
        # Automatically identify effective areas
        spatial_mask = (x.abs().sum(dim=1, keepdim=True) > 1e-6 ).float()  # [B, 1, H, W]
        mask_expand = spatial_mask.expand_as(x)  # [B, C, H, W]
        
        # Calculate the masked average
        valid_count = mask_expand.sum(dim=(2, 3), keepdim=True).clamp(min=1)  # [B, C, 1, 1]
        masked_sum = (x * mask_expand).sum(dim=(2, 3), keepdim=True)  # [B, C, 1, 1]
    ...
    def forward(self, x):
        ...
        y = self.masked_global_avg_pool(x).view(b, c)
